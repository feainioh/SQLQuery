...
internal class Capture : ISampleGrabberCB, IDisposable
{
    ...
    public ICaptureGraphBuilder2 m_capGraph = null;
    ...
    private void _Capture(int iDeviceNum, int iFrameRate, int iWidth, int iHeight)
    {
        ...
        {
            throw new Exception("沒有檢測到" + (iDeviceNum + 1).ToString() + "號攝像頭！");
        }
    ...
    private void SetConfigParms(ICaptureGraphBuilder2 capGraph, IBaseFilter capFilter, int iFrameRate, int iWidth, int iHeight)
    {
        ...
        VideoInfoHeader v_infoHead = new VideoInfoHeader();
        Marshal.PtrToStructure(media.formatPtr, v_infoHead);

        // if overriding the framerate, set the frame rate
        if (iFrameRate > 0)
        {
            v_infoHead.AvgTimePerFrame = 10000000 / iFrameRate;
        }

        // if overriding the width, set the width
        if (iWidth > 0)
        {
            v_infoHead.BmiHeader.Width = iWidth;
        }

        // if overriding the Height, set the Height
        if (iHeight > 0)
        {
            v_infoHead.BmiHeader.Height = iHeight;
        }

        // Copy the media structure back
        Marshal.StructureToPtr(v_infoHead, media.formatPtr, false);
