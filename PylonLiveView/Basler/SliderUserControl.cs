    ...
    public partial class MySliderUserControl : UserControl
    {
        ...
        private void DeviceOpenedEventHandler()
        {
            ...
            try
            {
                 ...
                 */
                if (
                    !m_hNode.IsValid &&/* No node has been found using the provided name. */
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if ((Convert.ToInt32(textBox1.Text) < slider.Minimum)
                    || (Convert.ToInt32(textBox1.Text) > slider.Maximum))
                {
                    MessageBox.Show("輸入值超出範圍，請重新輸入!");
                    return;
                }
                valueChanged(Convert.ToInt32(textBox1.Text));
                slider.Value = Convert.ToInt32(textBox1.Text);
            }
            catch { }
        }
    }
